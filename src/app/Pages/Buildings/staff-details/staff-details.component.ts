...
export class StaffDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.buildID = localStorage.getItem('ProjectID');
    ...
    var list = this.activatedRoute.params.subscribe(data => {
      ...
      if (this.paramID != null && this.paramID != undefined) {
        ...
        this.fmsservice.GetBuildingStaffByID(this.paramID).subscribe(data => {
          ...
          this.rateperhour = this.stafflist[0].rateperhour;
  ...
  InsertBuildingStaff() {
    ...
    this.fmsservice.GetTotalNoOfUsers().subscribe(data => {
      debugger
      this.SumCount = data[0].sumCount;
      let NoOFUsers = localStorage.getItem('NoOfUsers');
      if (this.SumCount >= NoOFUsers) {
        Swal.fire("Sorry!!! Your Are not Allowed To Add More Users..");
      } else {
        var man = {
        ...
        if (validate.length > 0) {
          ...
          this.fmsservice.InsertPhiliStaff(filter).subscribe(data => {
            debugger;
