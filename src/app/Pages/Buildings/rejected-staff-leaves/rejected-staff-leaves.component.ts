...
@Component({
  selector: 'app-rejected-staff-leaves',
  templateUrl: './rejected-staff-leaves.component.html',
  styleUrls: ['./rejected-staff-leaves.component.css']
})
export class RejectedStaffLeavesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (localStorage.getItem('LoginTypeID') == '7') {
      this.fmsservice.GetStaffLeaves(1).subscribe(
        res => {
          let temp: any = res
          this.StaffLeaves = temp.filter(x => x.buildingid == 109 && x.status == 'Rejected');
  ...
  public GetStaffLeaves(languageid) {
    ...
    this.fmsservice.GetStaffLeaves(languageid).subscribe(
      res => {
        let temp: any = res
        this.StaffLeaves = temp.filter(x => x.buildingid == this.ProjectID && x.status == 'Rejected');
        this.Count = this.StaffLeaves.length;
        this.buildcallender(this.StaffLeaves)
      }
    )
  ...
  exporttoexcel() {
    ...
    const Export_to_excel_options = {
      ...
      filename: 'Staff Leaves'
      // headers: ['Column 1', 'Column 2', etc...] <-- Won't work with useKeysAsHeaders present!
    };
    const csvExporter = new ExportToCsv(Export_to_excel_options);
    csvExporter.generateCsv(this.StaffLeaves);
  }
