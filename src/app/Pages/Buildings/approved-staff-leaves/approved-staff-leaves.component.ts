...
@Component({
  selector: 'app-approved-staff-leaves',
  templateUrl: './approved-staff-leaves.component.html',
  styleUrls: ['./approved-staff-leaves.component.css']
})
export class ApprovedStaffLeavesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (localStorage.getItem('LoginTypeID') == '7') {
      this.fmsservice.GetStaffLeaves(1).subscribe(
        res => {
          let temp: any = res
          this.StaffLeaves = temp.filter(x => x.buildingid == 109 && x.status == 'Approved');
  ...
  public GetStaffLeaves(languageid) {
    ...
    this.fmsservice.GetStaffLeaves(languageid).subscribe(
      res => {
        let temp: any = res
        this.StaffLeaves = temp.filter(x => x.buildingid == this.ProjectID && x.status == 'Approved');
        this.Count = this.StaffLeaves.length;
        this.buildcallender(this.StaffLeaves)
      }
    )
