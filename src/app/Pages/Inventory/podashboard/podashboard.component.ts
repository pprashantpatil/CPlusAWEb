...
export class PodashboardComponent implements OnInit {
  ...
  public GetPODashboard(languageid) {
    ...
    this.fmsservice.GetPODashboard("2020-01-01", "2020-12-31", languageid).subscribe(
      res => {
        ...
        this.FilteredPOList = this.POList.filter(x => x.status == 'Pending' && x.buildingID == this.ProjectID);
  ...
  public FilterByVendors(evn) {
    ...
    if (evn.target.value == "none") {
      ...
      this.FilteredPOList = this.POList.filter(x => x.vendor == this.selectedVendor && x.status == 'Pending');
      this.Count = this.FilteredPOList.length;
    }
  }
