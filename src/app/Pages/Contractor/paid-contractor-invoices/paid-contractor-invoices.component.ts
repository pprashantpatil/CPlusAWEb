...
@Component({
  selector: 'app-paid-contractor-invoices',
  templateUrl: './paid-contractor-invoices.component.html',
  styleUrls: ['./paid-contractor-invoices.component.css']
})
export class PaidContractorInvoicesComponent implements OnInit {
  ...
  public fliterbuilding(event) {
    ...
    if (this.buildingid == 0) {
      this.fmsService.GetContractorInvoice().subscribe(data => {
        debugger
        let temp: any = data;
        this.ProjectAssignedLists = temp;
      })
    }
    else {
      this.fmsService.GetContractorInvoice().subscribe(data => {
        debugger
        let temp: any = data;
        this.ProjectAssignedLists = temp.filter(x => x.projectID == this.buildingid);
      })
    }
  }
