...
@Component({
  selector: 'app-project-expenses-dash',
  templateUrl: './project-expenses-dash.component.html',
  styleUrls: ['./project-expenses-dash.component.css']
})
export class ProjectExpensesDashComponent implements OnInit {
  pickeroptions: any
  CategoryLists: any
  ...
  public getExpenses() {
    ...
    this.fmsservice.GetProjectExpense().subscribe(data => {
      debugger
      let temp: any = data.filter(x => x.expenseType == 'Inventory' && x.buildingid == localStorage.getItem('ProjectID'));
  ...
  public GetExpenseType(event) {
    ...
    if (this.ExpenseType == 0) {
      ...
      this.fmsservice.GetProjectExpense().subscribe(data => {
        debugger
        let temp: any = data.filter(x => x.expenseType == 'Inventory' && x.buildingid == localStorage.getItem('ProjectID'));
        this.ExpenseList = temp.filter(x => x.expenseType == this.ExpenseType);
        let total1 = 0;
        this.ExpenseList.forEach(element => {
          total1 += element.totalExpense;
        });
  ...
  public GetStaffTypeID(event) {
    ...
    this.fmsservice.GetStaff(1).subscribe(
      res => {
        debugger;
        let temp: any = res
        this.StaffList = temp.filter(x => x.staffTypeID == this.StaffType && x.companyID == localStorage.getItem('CompanyID'));
      }
    )
    if (this.StaffType == 0) {
      this.getExpenses();
