...
@Component({
  selector: 'app-financesetteledpayments',
  templateUrl: './financesetteledpayments.component.html',
  styleUrls: ['./financesetteledpayments.component.css']
})
export class FinancesetteledpaymentsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.fmsService.Get_SubContractor().subscribe(data => {
      debugger
      let temp: any = data.filter(x => x.companyID == localStorage.getItem('CompanyID'));
      this.Contractorlist = temp;
    })
    this.fmsService.GetBuildinglist(1).subscribe(
      res => {
        debugger;
        this.BuildingList = res.filter(x => x.companyID == localStorage.getItem('CompanyID'));
      }
    )
    ...
    this.fmsService.GetContractorInvoice().subscribe(data => {
      debugger
      let temp: any = data;
      this.ProjectAssignedLists = temp.filter(x => x.paidAmount != 0 && x.companyID == localStorage.getItem('CompanyID'));
    })
