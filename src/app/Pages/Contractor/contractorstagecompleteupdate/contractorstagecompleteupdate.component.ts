...
@Component({
  selector: 'app-contractorstagecompleteupdate',
  templateUrl: './contractorstagecompleteupdate.component.html',
  styleUrls: ['./contractorstagecompleteupdate.component.css']
})
export class ContractorstagecompleteupdateComponent implements OnInit {
  ...
  public GetContractorID(event) {
    ...
    if (this.Contractorid == 0) {
      ...
      this.fmsService.GetDailyUpdateMenu().subscribe(data => {
        debugger
        let temp: any = data;
        this.ProjectAssignedLists = temp.filter(x => x.projectID == this.ProjectID && x.contractorID == this.Contractorid && x.uPdateID == 2);
      })
    }
  }
