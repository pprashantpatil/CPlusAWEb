...
@Component({
  selector: 'app-contractor-staff-leaves-dash',
  templateUrl: './contractor-staff-leaves-dash.component.html',
  styleUrls: ['./contractor-staff-leaves-dash.component.css']
})
export class ContractorStaffLeavesDashComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.GetContractorStaffLeaves(this.selectedlanguage1);
    ...
    if (this.LoginID == 4) {
      this.fmsservice.GetLeaveType(this.selectedlanguage1).subscribe(data => {
        debugger
        this.Leavetypelist = data.filter(x => x.companyID == this.CompanyID);
  ...
  public GetContractorStaffLeaves(languageid) {
    debugger
    this.fmsservice.GetContractorStaffLeaves(languageid).subscribe(
      res => {
        let temp: any = res
  ...
  approveleave(staffID) {
    ...
    this.fmsservice.UpdateContractorStaffLeaves(procs).subscribe(data => {
      debugger
      ...
      if (data != undefined) {
        ...
        debugger
        this.fmsservice.GetContractorStaffLeaves(this.selectedlanguage1).subscribe(
          res => {

            let temp: any = res
            this.StaffLeaves = temp;
          }
        )
      }
