...
@Component({
  selector: 'app-completed-punchlist',
  templateUrl: './completed-punchlist.component.html',
  styleUrls: ['./completed-punchlist.component.css']
})
export class CompletedPunchlistComponent implements OnInit {
  ...
  public GetPunch_List_Assign() {
    ...
    if (this.LoginTypeID == '1') {
      this.fmsservice.GetPunch_List_Assign().subscribe(data => {
        debugger
        this.projectlist = data.filter(x => x.staffID == this.StaffID && x.projectID == this.ProjectID && x.observation != null);
        this.Count = this.projectlist.length;
      })
    } else {
      this.fmsservice.GetPunch_List_Assign().subscribe(data => {
        debugger
        this.projectlist = data.filter(x => x.projectID == this.ProjectID && x.observation != null);
        this.Count = this.projectlist.length;
      })
    }
  ...
  exporttoexcel() {
    ...
    const Export_to_excel_options = {
      ...
      showLabels: true,
      showTitle: true,
      title: 'Completed Punch Lists',
      useTextFile: false,
      useBom: true,
      useKeysAsHeaders: true,
      filename: 'Completed Punch Lists'
